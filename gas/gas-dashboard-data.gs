  ...
  try {
    ...
    (propsResult.results || []).forEach(function(page) {
      ...
      if (chakou) {
        ...
        if (cStr <= today) {
          startedProperties.push({ name: shortName, date: (cMon+1)+'/'+cDay, _sort: cMon*100+cDay });
        } else if (cMon === currentMonth && cYear === thisYear) {
          monthlyStarts.push({ name: shortName, date: (cMon+1)+'/'+cDay, _sort: cMon*100+cDay });
        }
      }
      if (hiki) {
        ...
        var shortName2 = displayName;
        if (hStr <= today) {
          deliveredProperties.push({ name: shortName2, date: (hMon+1)+'/'+hDay, _sort: hMon*100+hDay });
        } else if (hMon === currentMonth && hYear === thisYear) {
          monthlyDeliveries.push({ name: shortName2, date: (hMon+1)+'/'+hDay, _sort: hMon*100+hDay });
        }
      }
    });
